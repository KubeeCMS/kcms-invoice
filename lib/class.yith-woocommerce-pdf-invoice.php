...
//			}
		...
		public function is_visible_document_notes( $document ) {
			...
			if ( $document instanceof YITH_Credit_Note ) {
				$is_visible = 'yes' === ywpi_get_option( 'ywpi_show_invoice_notes', $document );
			} elseif ( ywpi_document_behave_as_invoice( $document ) ) {
				$is_visible = 'yes' === ywpi_get_option( 'ywpi_show_credit_note_notes', $document );
			} elseif ( $document instanceof YITH_Shipping ) {
				$is_visible = 'yes' === ywpi_get_option( 'ywpi_packing_slip_show_notes', $document );
			}
		...
		function is_visible_document_footer( $document ) {
			...
			if ( $document instanceof YITH_Credit_Note ) {
				$is_visible = 'yes' === ywpi_get_option( 'ywpi_show_credit_note_footer', $document );
			} elseif ( ywpi_document_behave_as_invoice( $document ) ) {
		...
		public function attach_documents_to_email( $attachments, $status, $object ) {
            ...
            if ( $status == 'customer_refunded_order' || $status == 'customer_partially_refunded_order' && 'manual' != ywpi_get_option( 'ywpi_invoice_generation' ) ){
		...
		private function get_next_number( $document, $type = 'invoice' ) {
			...
			if ( get_option( 'ywpi_enable_number_of_digits', 'no') == 'yes' ){
		...
		private function check_actual_invoice_number( $current_invoice_number, $document, $type ) {
			...
			if (  ! $document instanceof YITH_Credit_Note && 'yes' === get_option( 'ywpi_invoice_check_duplicated', 'yes' ) ){
				...
				if ( 'yes' === get_option( $reset_option ) ) {
					...
					$stored_invoice_numbers = $wpdb->get_col( $wpdb->prepare( "
					SELECT DISTINCT pm.meta_value FROM {$wpdb->postmeta} pm
					LEFT JOIN {$wpdb->posts} p ON p.ID = pm.post_id
					WHERE pm.meta_key = %s
					AND p.post_date > %s

					", $key, $first_day_of_year ) );

				}
				else{
					$stored_invoice_numbers = $wpdb->get_col( $wpdb->prepare( "SELECT DISTINCT pm.meta_value FROM {$wpdb->postmeta} pm WHERE pm.meta_key = %s", $key ) );
				}

				if ( ! empty( $stored_invoice_numbers ) && in_array( $current_invoice_number, $stored_invoice_numbers ) ){
